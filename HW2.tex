...
\begin{proof}
    ...
    % (\text{Skip one trace transformation})    &= \mathbb{E}\left[tr\left(\left(\mathbf{X}^{T} \mathbf{X}\right)^{-1} x_{0}x_{0}^{T}\left(\mathbf{X}^{T} \mathbf{X}\right)^{-1} \mathbf{X}^{T} \epsilon\epsilon^{T}\mathbf{X}\right)\right]\\
